...
<div align=center>
    <img src="img/Web-crawler.png" />
</div>

网页爬虫是一种在互联网上进行内容获取的机器人，它是搜索引擎和许多其他网站的基本工具。

网页爬虫是自动工作的，不需要人们进行交互。爬虫的复杂性可能会有很大的不同，一些爬虫就如相当复杂的软件（就像搜索引擎一样）。

爬虫在工作时会使用正在访问的网页资源，因此，公共网站都会有一种的机制来告诉爬虫哪些部分才可以爬取，或者告诉爬虫不要爬取任何内容。这些都可以通过使用`robots.txt (robots exclusion standard)`（机器人排除标准）来完成。

当然，由于它只是一个标准，`robots.txt`无法防止未经允许的爬虫爬取网站。比如一些恶意机器人包括电子邮件收集器，垃圾邮件和恶意软件。

以下是一些`robots.txt`的例子：
...
鉴于互联网的广泛性，即使是迄今为止最强大的爬取工具，也无法爬取所有的内容。这就是为什么要使用`selection policy`选择性政策来爬取最相关的部分。同样，互联网瞬息网变，因此爬虫必须采取`freshness policy`新鲜度政策来计算是否需要再次访问网站。而且，由于爬虫可以很容易产生大量请求，增加服务器负担，因此有一个`politeness policy`礼貌政策。大多数的爬虫使用20秒到3-4分钟的时间间隔轮询服务器，以避免在服务器上产生负载。


## Conclusion 总结
你现在应该可以更好地了解HTTP的工作原理，而且还有请求，响应和状态代码。HTTP利用硬件和软件不同的整体基础来实现它作为应用层传输协议的功能。
